            ...
            <div class="table-responsive" id="printable">
                <table class="table m-0 table-bordered">
                    ...
                    <tr>
                        ...
                        <th style="width: 20%; text-align: center; vertical-align: middle;">Mission</th>
                    ...
                    <?php
                    $i = 1;
                    $qry = $conn->query("SELECT c.mission_id, c.name, GROUP_CONCAT(CONCAT(p.id, '|', p.name) SEPARATOR '||') as projects
                                         FROM mission c 
                                         LEFT JOIN project_list p ON p.mission_id = c.mission_id
                                         GROUP BY c.mission_id");
                    while ($row = $qry->fetch_assoc()):
                    ?>
                        <tr>
                            <td style="width: 10%; vertical-align: middle;"><span class="large-font"><?php echo $i++ ?></span></td>
                            <td class="mission-column" style="width: 20%; vertical-align: middle;"><span class="large-font"><?php echo ucwords($row['name'] ?? '') ?></span></td>
...
<script>
    $('#print').click(function(){
        ...
        var _d = "<p class='text-center'><b>Rapport des projets par mission en date du (<?php echo date('F d, Y') ?>)</b></p>";
        _p.prepend(_d);
        _p.prepend(_h);
